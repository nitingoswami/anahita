...
{	
    ...
    {    	
        //Lock the below script to be excuted by one process at a time
        // check for lock file present
        if(file_exists("lock.txt")){
               
    	       throw new AnErrorException('Unexpected error when saving user. Please try after few seconds');
               
        }else{ 
        
         $content = "Lock start";
         $fp = fopen("lock.txt","wb");
         fwrite($fp,$content);
         fclose($fp); 
